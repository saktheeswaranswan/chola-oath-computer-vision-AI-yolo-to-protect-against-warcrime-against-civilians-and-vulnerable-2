...
inner_dome_radius = 300      # Inner dome (video-textured base)
outer_dome_radius = 320      # Outer dome (physical structure)
...
outer_entry_points = []  # When crossing into outer dome
inner_entry_points = []  # When crossing into inner dome

# Impact markers on the base (orange, shimmering spheres)
...
class Projectile:
    def __init__(self, pos, vel):
        ...
        self.marker_created = False  # Ensure one impact marker per projectile

    def update(self, dt):
        self.pos += self.vel * dt
        self.trail.append(tuple(self.pos))
        
        # Check for crossing dome boundaries (dome center = origin)
        ...
        # create an impact marker with explosion radius between 4 and 5.
        ...
        # Deactivate projectile when it reaches or falls below the base.
        if self.pos[1] <= 0:
            self.active = False
...
def get_zone_color(pos):
    """
    Return color based on projectile's position:
      - Green when outside the inner dome (base not yet entered)
      - Red when inside the inner dome (base)
    """
    d = np.linalg.norm(np.array(pos))
    if d > inner_dome_radius:
        return (0, 1, 0)  # Green
    else:
        return (1, 0, 0)  # Red
...
def spawn_projectile():
    # 🚀 Spawn a projectile from a spherical shell with varied range and height.
    # Spawn radius between outer_dome_radius+20 and outer_dome_radius+100.
    spawn_radius = random.uniform(outer_dome_radius + 20, outer_dome_radius + 100)
...
def draw_impact_markers():
    """
    Draw orange, shimmering spheres (impact explosions) on the inner dome base.
    Each sphere is drawn on the base (y=0) and a red line is drawn perpendicular (straight up)
    from the sphere's center for a height of 11 units.
    """
    # Disable depth testing so markers overlay the base
    ...
    for marker in impact_markers:
        pos, radius = marker
        glPushMatrix()
        # Force y-coordinate to 0 to plot on the base (webcam feed)
        glTranslatef(pos[0], 0, pos[2])
        gluSphere(quadric, radius, 16, 16)
        # Draw a red line straight up from the sphere (height = 11)
        glLineWidth(4)
        glColor3f(1, 0, 0)
        glBegin(GL_LINES)
        glVertex3f(0, 0, 0)
...
def render_scene():
    ...
    # Draw outer dome and inner dome (video-textured base)
    ...
    # Draw projectile trails and active rockets 🚀
    for proj in projectiles:
        draw_projectile_trail(proj)
        if proj.active:
            draw_projectile(proj)
    
    # Draw permanent markers and impact markers on the base
...
def main():
    ...
    pygame.display.set_caption("🚀 Iron Dome Simulation: High-Speed Trajectories, Explosions & Red Lines")
    glEnable(GL_DEPTH_TEST)
