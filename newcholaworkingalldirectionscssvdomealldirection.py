...
outer_entry_points = []  # When crossing into outer dome (if needed)
inner_entry_points = []  # When crossing into inner dome (if needed)
...
class Projectile:
    ...
    def update(self, dt):
        ...
        # Deactivate projectile once it reaches or falls below the base.
        if self.pos[1] <= 0:
            self.active = False
...
def get_zone_color(pos):
    ...
    Return color based on projectile's position:
      - Green when outside the inner dome (not yet entered the base)
...
def spawn_projectile():
    ...
    # Choose a spawn radius between (outer_dome_radius + 20) and (outer_dome_radius + 100)
...
def draw_impact_markers():
    """Draw orange, shimmering spheres (impact explosions) on the inner dome base.
       Markers are forced to y=0 so they plot on the webcam feed base."""
...
def main():
    ...
    pygame.display.set_caption("🚀 Iron Dome Simulation: High-Speed Trajectories & Explosions")
    glEnable(GL_DEPTH_TEST)
