...
# Permanent markers for dome entries (if desired)
...
impact_markers = []  # List of tuples: (position, radius)
...
class Projectile:
    def __init__(self, pos, vel):
        ...
        self.marker_created = False  # To ensure we create one marker only
    ...
    def update(self, dt):
        ...
        # When projectile reaches the base (y <= 0) and within the inner dome circle,
        ...
        # Deactivate projectile when it falls to or below the base.
        if self.pos[1] <= 0:
            self.active = False
...
def draw_permanent_markers():
    """Draw permanent yellow markers for dome entries (if desired)."""
    glPointSize(10)
    glColor3f(1, 1, 0)
    glBegin(GL_POINTS)
...
def draw_impact_markers():
    ...
    # Enable lighting for a shimmering effect
    glEnable(GL_LIGHTING)
    glEnable(GL_LIGHT0)
    # Set material properties for an orange, emissive look
...
def render_scene():
    ...
    # Draw permanent markers (if desired)
    draw_permanent_markers()
    # Draw impact markers (orange spheres at dome base)
    draw_impact_markers()
...
def main():
    ...
    pygame.display.set_caption("🚀 Transparent Domes with High-Speed Trajectories, Impact Markers & Permanent Markers")
